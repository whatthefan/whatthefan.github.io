...
q={int(k):np.array(v) for k,v in json.load(open('placa_quad_f.json')).items()}
ks=sorted(q)
...
os.makedirs('capa/cuf2',exist_ok=True)
for k in range(1,918):
    f='capa/cuf/%04d.jpg'%k; im=cv2.imread(f)
    if k in q:
    ...
    cv2.imwrite('capa/cuf2/%04d.jpg'%k,im,[cv2.IMWRITE_JPEG_QUALITY,93])
print('ok',len(q))
