"""Sonido del video "Piedra, papel... ¡reseñas!" (9,5 s). Los segundos son
los de render(t) en escena.html (los monta ../video.py). Utilidades,
...
a.poner(pads, a.pad(notas, 1.4, 1800), t0, .8); a.poner(melodia, a.campana(69, 1.4), t0)
a.poner(fx, m('whoosh-cinematic.mp3'), 3.62, .8)                   # entra la placa
a.poner(fx, m('whoosh.mp3', -2), 3.8, .6)                           # entra el movil
for i in range(3):
    a.poner(fx, m('ping.mp3', 4 * i), 4.3 + i * .2, .5)             # NFC
for i in range(5):
    a.poner(fx, m('kenney-glass_004.wav', [0, 2, 4, 7, 9][i]), 4.55 + i * .18, .8, pan=-.5 + .25 * i)
a.poner(fx, m('sparkle.mp3'), 5.5, .7)
a.poner(fx, m('pop.mp3', 2), 5.8, .6)
a.poner(fx, m('kenney-confirmation_004.wav'), 7.1, .8)
...
fx = a.reverb(fx, .22)
a.guardar(a.master(bajos + .9 * pads + .95 * bateria + .8 * melodia + 1.1 * fx, cola=.8), V + 'audio.wav')
a.guardar(a.master(fx, cola=.8), V + 'audio-solo-efectos.wav')
