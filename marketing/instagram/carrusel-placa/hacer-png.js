...
// Desde la raiz del repo:  node marketing/instagram/carrusel-placa/hacer-png.js
const path = require('path'), fs = require('fs');
const { chromium } = require(require('child_process').execSync('npm root -g').toString().trim() + '/playwright');
const D = 'marketing/instagram/carrusel-placa/diapositivas/';
(async () => {
  ...
  for (const f of fs.readdirSync(D).filter(f => f.endsWith('.html')).sort()) {
    ...
    await pg.screenshot({ path: D + 'PLEA5E-carrusel-' + f.replace('.html', '.png') });
  }
  await b.close();
})();
